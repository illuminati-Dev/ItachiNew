...
from SaitamaRobot import BOT_USERNAME as BOT_NAME, DEV_USERS as OWNER, pbot as anibot, DOWN_PATH, EVENT_LOGS as LOG_CHANNEL_ID
from SaitamaRobot.utlis.db import get_collection
from SaitamaRobot.utlis.helper import (
...
from SaitamaRobot.modules.anibotanilist import auth_link_cmd, code_cmd, logout_cmd
...
@anibot.on_message(filters.command(['start', f'start{BOT_NAME}']))
@control_user
async def start_(client: anibot, message: Message, mdata: dict):
    ...
    if gid==user:
        if user not in OWNER and not (await USERS.find_one({"id": user})):
            ...
            await clog("Makimabot", f"New User started bot\n\n[{usertitle}](tg://user?id={user})\nID: `{user}`", "NEW_USER")
...
@anibot.on_message(~filters.private & filters.command(['anidisabled', f'anidisabled{BOT_NAME}']))
@control_user
...
@anibot.on_message(filters.user(OWNER) & filters.command(['dbcleanup', f'dbcleanup{BOT_NAME}']))
@control_user
...
@anibot.on_message(filters.user(OWNER) & filters.command(['anibotstats', f'anibotstats{BOT_NAME}']))
@control_user
