...
from SaitamaRobot import DEV_USERS as OWNER, BOT_USERNAME as BOT_NAME, pbot as anibot
from SaitamaRobot.utlis.data_parser import (
...
from SaitamaRobot.utlis.helper import ANON_JSON, check_user, get_btns, AUTH_USERS, rand_key, clog, control_user, update_pics_cache
...
ANILIST_SECRET = "J2HnBTW8TlwWz5TEO4lUab2JpuyvqQeJYdoh46O8"

ANIME_IMG = "https://telegra.ph/file/ece3bee99dbad6b7e93ee.mp4"
MANGA_IMG = "https://telegra.ph/file/33d697122e04d6042b538.mp4"
CHARACTER_IMG = "https://telegra.ph/file/ce7e57797385abc3c0db5.mp4"
AIRING_IMG = "https://telegra.ph/file/d94be009204bd4927df39.mp4"
...
@anibot.on_message(filters.command(["anime", f"anime{BOT_NAME}"]))
@control_user
async def anime_cmd(client: anibot, message: Message, mdata: dict):
    ...
    if find_gc is not None and 'anime' in find_gc['cmd_list'].split():
        return
    if len(text)==1:
        k = await message.reply_animation(ANIME_IMG, caption="""Format : /anime < anime name >""", parse_mode=ParseMode.MARKDOWN)
...
@anibot.on_message(filters.command(["manga", f"manga{BOT_NAME}"]))
@control_user
async def manga_cmd(client: anibot, message: Message, mdata: dict):
    ...
    if find_gc is not None and 'manga' in find_gc['cmd_list'].split():
        return
    if len(text)==1:
        k = await message.reply_animation(MANGA_IMG, caption="""Format : /manga < manga name >""", parse_mode=ParseMode.MARKDOWN)
        await asyncio.sleep(20)
        return await k.delete()
    query = text[1]
    qdb = rand_key()
...
@anibot.on_message(filters.command(["character", f"character{BOT_NAME}"]))
@control_user
async def character_cmd(client: anibot, message: Message, mdata: dict):
    ...
    if find_gc is not None and 'character' in find_gc['cmd_list'].split():
        return
    if len(text)==1:
        k = await message.reply_animation(CHARACTER_IMG, caption="""Format : /character < character name >""", parse_mode=ParseMode.MARKDOWN)
        await asyncio.sleep(20)
        return await k.delete()
    query = text[1]
    qdb = rand_key()
...
@anibot.on_message(filters.command(["anilist", f"anilist{BOT_NAME}"]))
@control_user
...
@anibot.on_message(filters.command(["flex", f"flex{BOT_NAME}", "user", f"user{BOT_NAME}"]))
@control_user
...
@anibot.on_message(filters.command(["top", f"top{BOT_NAME}"]))
@control_user
async def top_tags_cmd(client: anibot, message: Message, mdata: dict):
    ...
    msg, buttons = result
    await message.reply_text(msg[0], reply_markup=buttons if buttons!='' else None)


@anibot.on_message(filters.command(["airing", f"airing{BOT_NAME}"]))
@control_user
async def airing_cmd(client: anibot, message: Message, mdata: dict):
    ...
    if find_gc is not None and 'airing' in find_gc['cmd_list'].split():
        return
    if len(text)==1:
        k = await message.reply_animation(AIRING_IMG, caption="""Format : /airing <anime name>""", parse_mode=ParseMode.MARKDOWN)
        await asyncio.sleep(20)
        return await k.delete()
    query = text[1]
    vars_ = {"search": query}
...
@anibot.on_message(filters.command(["auth", f"auth{BOT_NAME}"]))
@control_user
...
@anibot.on_message(~filters.private & filters.command(["aninsfw", f"aninsfw{BOT_NAME}"]))
@control_user
...
__✥NSFW toggle switches on filtering of 18+ marked content.__
"""
        sfw = "NSFW: Allowed"
...
@anibot.on_message(filters.command(["code", f"code{BOT_NAME}"]))
@control_user
...
@anibot.on_message(filters.command(["me", f"me{BOT_NAME}", "activity", f"activity{BOT_NAME}"]))
@control_user
...
@anibot.on_message(filters.command(["favourites", f"favourites{BOT_NAME}"]))
@control_user
...
@anibot.on_message(filters.command(["logout", f"logout{BOT_NAME}"]))
@control_user
...
from SaitamaRobot.utlis.data_parser import browse_

@anibot.on_message(filters.command(["browse", f"browse{BOT_NAME}"]))
@control_user
...
@anibot.on_message(filters.command(["gettags", f"gettags{BOT_NAME}", "getgenres", f"getgenres{BOT_NAME}"]))
@control_user
