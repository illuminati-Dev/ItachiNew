...
from SaitamaRobot import LOGGER, dispatcher, DRAGONS
...
from SaitamaRobot.modules.helper_funcs.decorators import makicmd, makimsg, makicallback
...
@makimsg((Filters.regex(r"^#[^\s]+")), group=-14)
@connection_status
...
@makimsg((Filters.regex(r"^/\d+$")), group=-16)
@connection_status
...
@makicmd(command='removeallnotes')
...
def __stats__():
    return f"𖣘 {sql.num_notes()} notes, across {sql.num_chats()} chats."
