...
async def nhentai_data(noombers):
...
async def nhentai(client, message):
    ...
    result = await nhentai_data(query)
    
    if result:
        title, tags, artist, total_pages, post_url, cover_image = result
        ...
        await message.reply_text("Couldn't fetch data from nhentai.")
